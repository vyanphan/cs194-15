...
void my_blur(float* out, int n, float* frame, int* radii) {
    ...
    //         for(int rr=r; rr<r+block_size; rr++) {
    //             for(int cc=c; cc<c+block_size; cc++) {
    //                 int rd = radii[rr*n+cc];
    //                 int num = 0;
    //                 float avg = 0;
    //                 for(int r2=max(0,rr-rd); r2<=min(n-1, rr+rd); r2++) {
    //                     for(int c2=max(0, cc-rd); c2<=min(n-1, cc+rd); c2++){
    //                         avg += frame[r2*n+c2];
    //                         num++;
    //                     }
    //                     out[rr*n+cc] = avg/num;
